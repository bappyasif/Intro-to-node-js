        ...
        .then(found => {
            if (found) {
                getStudentIdFromSession(req, res).then(userId => {
                    ...
                    createScedule(userId, deanId, [{ day: day }])

                    updateDeanFreeSlots(deanId, day)

                    return res.status(200).json({ msg: "booked appointnent", studentId: userId, deanId })
                })
            } else {
